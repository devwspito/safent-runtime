...
Base actualizada: runtime `6515a4c` y bloque Integraciones/diálogos, 2026-09-11. Inventario de código, no declaración de validación funcional. UI-01 sigue **parcial**. Community es una aplicación nativa: una revisión de su React en navegador NO valida el binario Tauri ni permisos del sistema operativo.
...
| `/tareas`, `/tareas?tab=programadas` | TasksView, CalendarView | Rediseño de root integrado; Office/Swarm/pixel ya retirados del frontend | API de dashboard/delegación y retiro backend de fábrica siguen siendo un bloque aparte; no simular un equipo por nombres |
...
| `/sistema?tab=memoria` | MemoriaView | Inventario y lectura parcial; Drawer compartido mejorado | Verificar carreras búsqueda/detalle/save, fallo detalle no editable, densidad y foco |
| `/sistema?tab=archivos` | ArchivosView | Inventario y lectura parcial; Drawer compartido mejorado | Verificar navegación/preview tardío, error preview distinguible de vacío, subida y selector nativo |
...
| --- | --- | --- |
| Preparación/instalación, fallo con reintento/copia de diagnóstico, reconexión, listo | `desktop/src/index.html`, render.ts, lifecycle.ts; salida generada en desktop/ui | Root informa 83 tests y build/typecheck en bloque nativo; recorrido de binario y permisos reales sigue pendiente. UiState usa preparing/failed/reconnecting/ready; no es otro router de negocio |
