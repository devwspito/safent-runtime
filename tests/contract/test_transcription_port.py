...
``src/hermes/training/testing/fake_transcription.py`` declara el shape del Protocol del
contract ``specs/002-.../contracts/transcription_port.py``.
...
def _import_protocol_from_spec():
    ...
    try:
        mod = __import__("transcription_port", fromlist=["*"])
    finally:
        sys.path.remove(str(_CONTRACT_DIR))
    return getattr(mod, "TranscriptionPort")
