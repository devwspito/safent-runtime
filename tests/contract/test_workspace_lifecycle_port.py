...
El contrato SDD (``specs/002-.../contracts/workspace_lifecycle_port.py``) NO se migró a
``src/`` (ausente en el checkout y en la imagen horneada); a diferencia de sus puertos
hermanos, este Protocol nunca aterrizó en ``hermes.workspace.domain.ports``. Mientras
falte, la verificación del Protocol se salta y sólo se valida el fake en memoria
``InMemoryWorkspaceLifecycle`` (que sí existe en ``src/`` y en la imagen).
...
def _load_port_protocol():
    ...
    try:
        from hermes.workspace.domain.ports.workspace_lifecycle_port import WorkspaceLifecyclePort
    except ImportError as exc:  # pragma: no cover - depende de artefactos de spec
        pytest.skip(
            f"WorkspaceLifecyclePort Protocol no presente en src (contrato spec-002 no migrado): {exc}"
        )
    return WorkspaceLifecyclePort
