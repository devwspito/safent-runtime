...
class TestSeedSelfHealOnReconnect:
    """A seed whose offline spawn dies (cold cache after an image update) must be
    re-warmed via the trusted-path prefetch and retried exactly once."""

    def test_seed_connect_failure_prefetches_and_retries(self, tmp_path, monkeypatch):
        seed_file = tmp_path / "mcp-servers.json"
        seed_file.write_text(json.dumps([
            {"server_id": "serena", "label": "x",
             "argv": ["uvx", "--from", "serena-agent==1.5.3", "serena", "start-mcp-server"]},
        ]), encoding="utf-8")
        monkeypatch.setattr(_drs, "_MCP_SEED_FILE", str(seed_file))
        monkeypatch.setattr(_drs, "_MCP_SEED_MARKER", str(tmp_path / "marker.json"))

        entry_argv = ["uvx", "--from", "serena-agent==1.5.3", "serena", "start-mcp-server"]
        neus = {"serena": {"command": entry_argv[0], "args": entry_argv[1:]}}
        cfg_mod = sys.modules["hermes_cli.config"]
        store = {"mcp_servers": neus}
        monkeypatch.setattr(cfg_mod, "load_config", lambda: store, raising=False)
        monkeypatch.setattr(cfg_mod, "save_config", store.update, raising=False)
        tool_mod = sys.modules["tools.mcp_tool"]
        monkeypatch.setattr(tool_mod, "_load_mcp_config", lambda: dict(neus), raising=False)
        monkeypatch.setattr(tool_mod, "register_mcp_servers", lambda _: [], raising=False)

        fake_server = MagicMock()
        fake_server.tools = [1] * 26
        connect = AsyncMock(side_effect=[RuntimeError("cold cache"), fake_server])
        prefetch = MagicMock()
        with (
            patch.object(_drs, "_mcp_connect", connect),
            patch.object(_drs, "_prefetch_mcp_package", prefetch),
            patch.object(_drs, "_scanner_can_analyze_argv", return_value=True),
        ):
            asyncio.run(_drs.reconnect_persisted_mcp_servers(MagicMock()))

        prefetch.assert_called_once_with("serena", entry_argv)
        assert connect.await_count == 2, "must retry the connect exactly once after prefetch"

    def test_user_added_entry_is_never_auto_prefetched(self, tmp_path, monkeypatch):
        monkeypatch.setattr(_drs, "_MCP_SEED_FILE", str(tmp_path / "missing.json"))
        monkeypatch.setattr(_drs, "_MCP_SEED_MARKER", str(tmp_path / "marker.json"))
        neus = {"mytool": {"command": "uvx", "args": ["some-published-pkg"]}}
        cfg_mod = sys.modules["hermes_cli.config"]
        store = {"mcp_servers": neus}
        monkeypatch.setattr(cfg_mod, "load_config", lambda: store, raising=False)
        monkeypatch.setattr(cfg_mod, "save_config", store.update, raising=False)
