...
export function renderNativeUpdater(element, value, commands) {
    const session = {};
    ...
    if (data.status !== 'available' || data.reason !== 'app_only')
        return;
    const api = commands ?? element.ownerDocument.defaultView?.__TAURI__?.core;
    if (!api?.invoke)
        return;
        ...
        try {
            if (pending) {
                ...
                if (!current())
                    return;
                if (result !== 'cancelled')
                    throw new Error('unconfirmed_install_result');
                status.textContent = 'Actualización cancelada. No se ha iniciado la instalación.';
            }
            else {
                const result = await api.invoke('check_native_update');
                if (!current())
                    return;
                if (!result || result.app_version !== data.app_version)
                    throw new Error('invalid_update_result');
