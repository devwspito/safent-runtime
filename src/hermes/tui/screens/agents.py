"""Profiles — native main persona and explicitly configured custom profiles.
...
default profile is never edited or deleted here. Conversation bindings own
selection; there is no global active agent to change from this pane.
"""
...
class AgentsPane(Pane):
    PANE_ID = "agents"
    TITLE = "Perfiles"
    SUBTITLE = "Perfil principal y perfiles personalizados, sujetos a políticas y aprobaciones."
    ...
    def build(self):
        ...
        table.add_columns("", "Perfil", "Rol", "Autonomía", "Tipo")
        yield table
        yield Static(
            Text(
                "n nuevo · e editar · d borrar  ·  "
    ...
    async def refresh_data(self) -> None:
        ...
        for a in self._agents:
            ...
            marker_style = PALETTE["amber"] if is_default else PALETTE["text_faint"]
            estado = "Principal" if is_default else "Personalizado"
            table.add_row(
                ...
                Text(estado, style=marker_style if is_default else PALETTE["text_muted"]),
                key=str(a.get("id")),
            )
    ...
    def action_new_agent(self) -> None:
        ...
        self.app.push_screen(
            FormModal("Nuevo perfil", fields, save_label="Crear",
                      note="Un perfil no amplía permisos ni evita las aprobaciones."),
            self._on_new_result,
        )
    ...
    async def _create(self, values: dict) -> None:
        try:
            ...
            self.notify(f"Perfil «{values.get('name')}» creado", timeout=3)
    ...
    def action_edit_agent(self) -> None:
        ...
        if agent.get("is_default"):
            self.notify(
                "El perfil principal está protegido: no se edita aquí.",
    ...
    async def _update(self, agent: dict, values: dict) -> None:
        try:
            ...
            self.notify("Perfil actualizado", timeout=3)
            await self.refresh_data()
        except Exception as exc:  # noqa: BLE001
    ...
    async def _delete(self, agent: dict) -> None:
        try:
            ...
            self.notify("Perfil borrado", timeout=3)
            await self.refresh_data()
        except Exception as exc:  # noqa: BLE001
