...
it.each(['mfa_required', 'invalid_totp', 'mfa_not_enrolled'])('does not refresh or replay an approval on %s', async code => {
  vi.resetModules()
  ...
  await expect(resolveApproval('proposal', 'once', { totp: '123456' })).rejects.toMatchObject({ code, status: 401 })
