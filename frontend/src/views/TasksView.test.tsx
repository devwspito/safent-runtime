...
describe('Community task dashboard', () => {
  ...
  it('shows a quarantined sync issue as attention without changing the local result or rerunning it', async () => {
    vi.mocked(getTaskDashboard).mockResolvedValue({ available: true, tasks: [{ ...task, enterprise_sync: { state: 'blocked', reason: 'execution_changed' } }], has_more: false })
    await render(); await click('Necesitan atención'); await click('Revisar campaña')
    expect(host.textContent).toContain('Sin sincronizar')
    expect(host.textContent).toContain('El encargo aparece vinculado a otra ejecución')
    expect(host.textContent).toContain('no repitas la tarea')
    expect(host.textContent).toContain('Informe listo')
    expect(host.querySelector('aside [role="status"]')).toBeTruthy()
    await click('Abrir conversación')
    expect(open).toHaveBeenCalledWith('chat-1')
  })
  it('shows durable pending telemetry honestly and does not add it to attention', async () => {
    vi.mocked(getTaskDashboard).mockResolvedValue({ available: true, tasks: [{ ...task, enterprise_sync: { state: 'pending' } }], has_more: false })
    await render(); await click('Revisar campaña')
    expect(host.textContent).toContain('Se reintentará automáticamente')
    await click('Necesitan atención')
    expect(host.textContent).toContain('No hay tareas que coincidan')
  })
  it.each([null, { state: 'synced' }, { state: 'blocked', reason: 'PRIVATE remote text' }])('rejects malformed telemetry without rendering raw errors', async enterprise_sync => {
    vi.mocked(getTaskDashboard).mockResolvedValue({ available: true, tasks: [{ ...task, enterprise_sync }], has_more: false } as never)
    await render()
    expect(host.textContent).toContain('No se pudo consultar el cuadro')
