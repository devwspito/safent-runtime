...
describe('SkillsView — hub install force (owner confirmation)', () => {
  ...
  it.each(['offline','unknown','empty-pass'])('does not install after an unverifiable scan: %s',async mode=>{
    if(mode==='offline') scanInstall.mockRejectedValue(new Error('unavailable'))
    ...
    act(()=>root.render(React.createElement(SkillsView)));await flush()
    typeInto(container.querySelector<HTMLInputElement>('#hub-search')!,'gitnexus')
    clickButton(container,t=>t==='Buscar');await flush();clickButton(container,t=>t==='Instalar');await flush()
    expect(installSkill).not.toHaveBeenCalled();expect(recordSecurityDecision).not.toHaveBeenCalled()
    expect(sileoError).toHaveBeenCalled()
  })
  it('reports search failure without calling it an empty result',async()=>{
    searchSkillsHub.mockRejectedValue(new Error('secret upstream'))
    act(()=>root.render(React.createElement(SkillsView)));await flush()
    typeInto(container.querySelector<HTMLInputElement>('#hub-search')!,'gitnexus')
    clickButton(container,t=>t==='Buscar');await flush()
    expect(container.querySelector('[role="alert"]')?.textContent).toContain('No se pudo consultar')
    expect(container.textContent).not.toContain('secret upstream')
  })
  it('does not install when a scan arrives after leaving the view',async()=>{
    let resolve!:(value:unknown)=>void;scanInstall.mockReturnValue(new Promise(r=>{resolve=r}))
    act(()=>root.render(React.createElement(SkillsView)));await flush()
    typeInto(container.querySelector<HTMLInputElement>('#hub-search')!,'gitnexus')
    clickButton(container,t=>t==='Buscar');await flush();clickButton(container,t=>t==='Instalar');await flush()
    act(()=>root.render(null));await act(async()=>resolve({...FAIL_SCAN,verdict:'PASS',requires_owner_approval:false}))
    expect(installSkill).not.toHaveBeenCalled();expect(recordSecurityDecision).not.toHaveBeenCalled()
  })
  it('does not offer a verified installed list when hub discovery fails',async()=>{
    listHubSkills.mockRejectedValue(new Error('offline'))
    act(()=>root.render(React.createElement(SkillsView)));await flush()
